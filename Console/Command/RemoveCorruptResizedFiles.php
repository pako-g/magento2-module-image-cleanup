...
class RemoveCorruptResizedFiles extends ConsoleCommand
{
    ...
    protected function configure(): void
    {
        ...
        $this->addOption(
            ...
            'Write the list of corrupt files to a .txt file instead of deleting them. '
            . 'Optionally provide a filename; defaults to var/corrupt_resized_files_<date>.txt.',
            false
        );
    ...
    {
        if ($files === []) {
            $output->writeln('<info>No corrupt files found, nothing to write.</info>');
            return Cli::RETURN_SUCCESS;
        }

        if ($filename === null || $filename === '') {
            $varDir = $this->directoryList->getPath(DirectoryList::VAR_DIR);
            $filename = $varDir . '/corrupt_resized_files_' . date('Y-m-d_H-i-s') . '.txt';
        }
        ...
        $output->writeln(sprintf(
            '<info>Written %d corrupt file paths to: %s</info>',
            count($files),
            $filename
        ));
